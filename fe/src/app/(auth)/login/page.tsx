...
import { AuthWebViewBottomSheet } from "@/components/shared/auth-webview-bottomsheet";
...
const LoginPageContent = () => {
  ...
  // iOS PWA 환경에서 iframe 내부인지 확인
  const isInIframe =
    typeof window !== "undefined" && window.self !== window.top;

  /**
   * @description iOS PWA 환경에서 /login 페이지를 웹뷰 바텀시트로 열기
   */
  useEffect(() => {
    const isIOSPWA = isIOSDevice() && isStandalone();

    // iOS PWA 환경이고, iframe 내부가 아니고, 이미 외부 링크로 열린 것이 아닌 경우
    if (
      isIOSPWA &&
      ...
      debug.log("iOS PWA 환경: /login 페이지를 웹뷰 바텀시트로 열기");
