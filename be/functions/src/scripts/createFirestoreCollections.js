...
const createFirestoreCollections = async () => {
  try {
    ...
    // ID 생성 - 루틴/소모임/TMI ID와 커뮤니티 ID를 동일하게 설정
    const routineId = `CP:${generateRandomId()}`;
    const gatheringId = `CP:${generateRandomId()}`;
    const tmiId = `CP:${generateRandomId()}`;

    // 커뮤니티 ID는 각각의 메인 ID와 동일하게 설정 (검색을 위해 중요!)
    const routineCommunityId1 = routineId; // 루틴 ID와 동일 - 검색을 위해 필수!
    const routineCommunityId2 = `CP:${generateRandomId()}`; // 추가 루틴 커뮤니티
    const routineCommunityId3 = `CP:${generateRandomId()}`; // 추가 루틴 커뮤니티
    const gatheringCommunityId1 = gatheringId; // 소모임 ID와 동일 - 검색을 위해 필수!
    const gatheringCommunityId2 = `CP:${generateRandomId()}`; // 추가 소모임 커뮤니티
    const gatheringCommunityId3 = `CP:${generateRandomId()}`; // 추가 소모임 커뮤니티
    const tmiCommunityId1 = tmiId; // TMI ID와 동일 - 검색을 위해 필수!
    const tmiCommunityId2 = `CP:${generateRandomId()}`; // 추가 TMI 커뮤니티
    const tmiCommunityId3 = `CP:${generateRandomId()}`; // 추가 TMI 커뮤니티
    ...
    for (const community of communities) {
      ...
      if (
        ...
        // 루틴 커뮤니티는 해당 루틴 ID를 참조
        refId = community.id === routineCommunityId1 ? routineId : routineId;
      } else if (
        community.id === gatheringCommunityId1 ||
        community.id === gatheringCommunityId2 ||
        community.id === gatheringCommunityId3
      ) {
        // 소모임 커뮤니티는 해당 소모임 ID를 참조
        refId =
          community.id === gatheringCommunityId1 ? gatheringId : gatheringId;
      } else if (
        community.id === tmiCommunityId1 ||
        community.id === tmiCommunityId2 ||
        community.id === tmiCommunityId3
      ) {
        // TMI 커뮤니티는 해당 TMI ID를 참조
        refId = community.id === tmiCommunityId1 ? tmiId : tmiId;
      }
    ...
    for (const communityId of routineCommunities) {
      ...
      await db
        ...
        .set({
          ...
          type: "ROUTINE_CERT",
          refId: communityId === routineCommunityId1 ? routineId : routineId, // 루틴 ID와 연동
          authorId: "user123",
          author: "루틴러",
    ...
    for (const communityId of gatheringCommunities) {
      ...
      await db
        ...
        .set({
          ...
          type: "GATHERING_REVIEW",
          refId:
            communityId === gatheringCommunityId1 ? gatheringId : gatheringId, // 소모임 ID와 연동
          authorId: "user123",
          author: "소모임러",
          communityPath: `communities/${communityId}`,
          title: "소모임 참여 후기 📚",
    ...
    for (const communityId of tmiCommunities) {
      ...
      await db
        ...
        .set({
          ...
          type: "TMI",
          refId: communityId === tmiCommunityId1 ? tmiId : tmiId, // TMI 프로젝트 ID와 연동
          authorId: "user123",
          author: "TMI러",
          communityPath: `communities/${communityId}`,
          title: "나만의 TMI 소개 🎨",
